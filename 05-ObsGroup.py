...
g = ioda.Engines.HH.createFile(
    name = "Example-GEMS2-IDOA.hdf5",
    mode = ioda.Engines.BackendCreateModes.Truncate_If_Exists)


# We will need to open the GEMS2 file in order to get the geolocation
# and brightness temperature data
...
gems2 = h5py.File(gems2_filepath, 'r')

# Main beam antenna temperature and standard deviation from gems2 file
tbDset = gems2['radiometric_data']['main_beam_antenna_temperatures']
tbErrDset = gems2['radiometric_data']['main_beam_antenna_temperature_standard_deviations']

# Geolocation data from gems2 file
latDset = gems2['geolocation_data']['latitude']
lonDset = gems2['geolocation_data']['longitude']
...
tbErrName = "ObsError/main_beam_antenna_temperatures"
...
tbErrVar = g.vars.create(tbErrName, ioda.Types.float, scales=[nlocsVar, nchansVar], params=p1)
...
tbVar.atts.create("long_name", ioda.Types.str, [1]).writeDatum.str("brightness temperature")
...
tbErrVar.atts.create("coordinates", ioda.Types.str, [1]).writeDatum.str("longitude latitude nchans")
tbErrVar.atts.create("long_name", ioda.Types.str, [1]).writeDatum.str("brightness temperature sd")
tbErrVar.atts.create("units", ioda.Types.str, [1]).writeDatum.str("K")
tbErrVar.atts.create("valid_range", ioda.Types.float, [2]).writeVector.float([100.0, 400.0])
...
lonVar.atts.create("long_name", ioda.Types.str, [1]).writeDatum.str("longitude")
...
tbErrVar.writeNPArray.float(tbErrData)
